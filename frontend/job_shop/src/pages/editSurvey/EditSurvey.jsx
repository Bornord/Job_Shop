...
export default function EditSurvey() {
    ...
    const [foundQuestion,setFoundQuestion] = useState({question:{},hasBeenFound:false});
    const searchQuestion = (question) =>{
        setFoundQuestion({question:testQuestion,hasBeenFound:true});
    }

    const getFoundQuestionText = ()=>{
        const nb = foundQuestion.question.responses
            .filter((response)=>response.isSelected).length
        let text = "Ajouter après "
        if(nb > 0){
            const selectedResponse = foundQuestion.question.responses
                .filter((response)=>response.isSelected)
                .reduce((prev,response)=>prev+(prev !== ""?", ":"")+response.placeholder,"")
            text += (nb == 1 ? "la réponse ":"les réponses ") + selectedResponse
        }else{
            text += "la question "
        }
        return text
        ...
        const postQuestion = async () =>{
            try{
                const res = await api.post("/addQuestionToQuestion",{
                    ...
                    addedToResponses: foundQuestion.question?.responses != "" ?foundQuestion.question?.responses
                    .filter((response)=>response.isSelected):"",
		...
		<div className='edit-survey'>
			...
			>
                ...
                <div className='choice-switch'>
                    ...
                    <div className="choice-switch-item">
                        <input type="radio" id="responses" name="choice" value="responses"
                            onChange={(e)=> {setOnText(false)}}/>
                ...
                {!isAText && 
                    <>
                        {responses.map((response)=>{
                            return <div key={response} className="response">
                                {response}
                                <div onClick={(e)=>
                        ...
                        <div className="input-container ic1">
                            <input
                                ...
                                onKeyDown={(e)=>{
                                    if(e.key === 'Enter') {
                                        setResponses((prev)=>{
                                            if(prev.includes(currentResponse)) return prev
                            ...
                            />
                            <div className="cut-question"></div>
                            <label htmlFor="response" className="placeholder">
                                Réponse :
                            </label>
                        </div>
                    </>
                ...
                <br />
                <SelectButton2 onClick={(e)=>{handleAddToEnd()}}>Ajouter à la fin du questionnaire</SelectButton2>
                
                <div className='linetext' onClick={
                    (e)=>{
                ...
                {isSearchBarVisible && 
                    <>
                        <div className="input-container ic1">
                            ...
                            />
                            <div className="cut-question"></div>
                            <label htmlFor="search-question" className="placeholder">
                                Question désirée :
                            </label>
                        </div>
                        {
                            ...
                            <>
                                ...
                                <br />
                                {foundQuestion.question?.responses != "" ? foundQuestion.question?.responses.map(
                                    (response)=>{
                                        const active = response.isSelected ? "active-response":""
                                        return <div key={response.placeholder} className={'response '+active} 
                                            onClick={
                                                (e)=>{
                                                    setFoundQuestion((prev)=>{
                                                        let {question} = prev
                                                        const responses = question.responses.map((r)=>{
                                                            if(r.placeholder == response.placeholder){
                                                                r.isSelected = !r.isSelected
                                                            }
                                                            return r
                                                        })
                                                        return {...prev,question:{...question,responses:responses}}
                                                    })
                                                }
                                            }
                                        >{response.placeholder}</div>
                                    }
                                ) : ""}
                                <br />
