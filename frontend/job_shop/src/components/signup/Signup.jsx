			...
			>
				{inputs
					...
					.map((input) => {
						const i = inputs.indexOf(input);
						return (
							<div key={input.id} className="input-container ic1">
								<input
									id={input.id}
									type={input.type}
									...
									placeholder=" "
									onChange={(e) => {
										handleInputs(input.id, e.target.value);
									}}
								/>
				...
				<SelectButton2 className={'submit-button'} type="submit">
					S'inscire
				</SelectButton2>
			</form>
			<div className="pages">
				{inputs.length > numberOfInputPerPage &&
					pages.map((page) => {
