...
import { SIGNUPASRECRUITER, SIGNUPASSTUDENT,signin } from '../../logic/features/user';
import './Signup.scss';

function Signup({status,inputs,title,subtitle,isValid}) {
    const numberOfInputPerPage = 3
	...
	const handleSubmit = () => {
		...
		const transformed = state.reduce((acc,current)=>{
			return {...acc,[current.id]:current.value }  
		} ,{} )
		transformed.error = handleError
